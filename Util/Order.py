...
def processxml(template, delimiter, orderid, targetbreitelb, maxhoehelb, patternImg):
    ...
    for key in keys:
        with open(file[0], "r", encoding="UTF-8") as f:
            ...
            if key[2] == "zuordnung":
                ...
                try:
                    ...
                    return
                erg = str(
                    Util.Imageprocessing.getscale(targetbreitelb, maxhoehelb, img)[0]
                )
                ...
                img = Image.open(img)
                erg = str(
                    Util.Imageprocessing.getscale(targetbreitelb, maxhoehelb, img)[1]
                )
                eval(key[0]).set(key[1], erg)
        ...
        if not patternImg == None:
            eval(keys[0][0]).set(keys[0][1], str(os.getcwd()) + "\\Util\\pattern.png")
